...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
  ...
  const staticPages=["/en","/th","/en/villas","/th/villas","/en/policies","/th/policies"].map(url=>({url:`${origin}${url}`,changeFrequency:"daily" as const,priority:url.endsWith("policies")?.5:url.endsWith("villas")?.9:1}));
