...
import { localizedInputPatch, localizedInputValue } from "../localized-input";
...
export function DetailsSection({ draft, setDraft }: { draft: VillaEditorDraft; setDraft: SetDraft }) {
  ...
  return <div className="min-w-0 space-y-4">
    <section className={`${section} grid gap-4`}><SectionTitle title={copy("Identity", "ข้อมูลระบุตัวตน")} detail={copy("Edit the public villa content for the selected admin language.", "แก้ไขเนื้อหาวิลล่าสาธารณะสำหรับภาษาผู้ดูแลที่เลือก")} className="" />
      <AdminField label={copy("Villa name", "ชื่อวิลล่า")} value={localizedInputValue(locale, draft.villa.nameEn, draft.villa.nameTh)} onChange={(e) => setDraft((current) => ({ ...current, villa: { ...current.villa, ...localizedInputPatch(locale, "nameEn", "nameTh", e.target.value) } }))} />
      <div className="grid gap-4 border-t border-[#eee8de] pt-4"><SectionTitle title={copy("Description", "คำอธิบาย")} detail={copy("Enter the description in English or Thai, then set the standard arrival and departure times.", "กรอกคำอธิบายเป็นภาษาอังกฤษหรือไทย จากนั้นกำหนดเวลาเข้าพักและออกจากที่พักมาตรฐาน")} className="" />
        <AdminTextarea label={copy("Description", "คำอธิบาย")} value={localizedInputValue(locale, draft.villa.descriptionEn, draft.villa.descriptionTh)} onChange={(e) => setDraft((current) => ({ ...current, villa: { ...current.villa, ...localizedInputPatch(locale, "descriptionEn", "descriptionTh", e.target.value) } }))} rows={7} />
...
export function PricingSection({ draft, setDraft }: { draft: VillaEditorDraft; setDraft: SetDraft }) {
  const { locale, copy } = useAdminLocale();
  const emptyRate: Omit<RateDraft, "key"> = { labelEn: "", labelTh: "", startDate: "", endDate: "", nightlyPriceThb: 0 };
  ...
  function openRateForm(rate?: RateDraft) {
    if (rate) {
      const preset = RATE_PRESETS.find((item) => item.labelEn === rate.labelEn && item.recurringDay === rate.recurringDay && (item.recurringDay || (item.startDate === rate.startDate && item.endDate === rate.endDate)));
      setForm({ ...rate }); setEditingKey(rate.key); setPresetSearch(preset ? localizedInputValue(locale, preset.labelEn, preset.labelTh) : localizedInputValue(locale, rate.labelEn, rate.labelTh)); setSelectedPreset(preset?.key ?? "custom"); setCustomDates(!preset);
    } else {
  ...
  function choosePreset(key: string) {
    ...
    if (key === "custom") { setCustomDates(true); setPresetSearch(copy("Custom dates", "กำหนดวันที่เอง")); setForm((current) => ({ ...current, labelEn: "", labelTh: "", startDate: "", endDate: "", recurringDay: undefined })); return; }
    const preset = RATE_PRESETS.find((item) => item.key === key);
    if (!preset) return;
    setCustomDates(false); setPresetSearch(localizedInputValue(locale, preset.labelEn, preset.labelTh)); setForm((current) => ({ ...current, labelEn: preset.labelEn, labelTh: preset.labelTh, startDate: preset.startDate ?? "", endDate: preset.endDate ?? "", recurringDay: preset.recurringDay }));
  }
  function saveRate() {
    if (!localizedInputValue(locale, form.labelEn, form.labelTh).trim()) { setFormError(copy("Choose a preset or enter a custom rate name.", "เลือกค่าที่ตั้งไว้หรือกรอกชื่อราคาแบบกำหนดเอง")); return; }
    if (!form.recurringDay && (!form.startDate || !form.endDate || form.endDate <= form.startDate)) { setFormError(copy("Choose a valid date range.", "เลือกช่วงวันที่ที่ถูกต้อง")); return; }
  ...
  return <div className="space-y-4"><AdminNotice tone="info" title={copy("Rate order controls precedence", "ลำดับราคากำหนดความสำคัญ")}>{copy("When rates overlap, the rate closest to the top wins. Drag rows into the order you want; weekday and weekend pricing applies when no special rate matches.", "เมื่อราคาทับซ้อนกัน ราคาที่อยู่บนสุดจะถูกใช้ก่อน ลากแถวเพื่อจัดลำดับ ราคาวันธรรมดาและวันหยุดสุดสัปดาห์จะใช้เมื่อไม่มีราคาพิเศษตรงกัน")}</AdminNotice>
    ...
    <section className={section}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><SectionTitle title={copy("Special rates", "ราคาพิเศษ")} detail={copy("Rank 1 wins when rates overlap. Drag rows to change their ranking.", "ลำดับ 1 จะถูกใช้เมื่อราคาทับซ้อนกัน ลากแถวเพื่อเปลี่ยนลำดับ")} className="" />{!showRateForm ? <AdminButton variant="secondary" onClick={() => openRateForm()}><Plus size={14} /> {copy("Add rate", "เพิ่มราคา")}</AdminButton> : null}</div>
      {showRateForm ? <div className="mt-4 rounded-xl border border-[#d9ddd9] bg-[#f8faf8] p-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="relative sm:col-span-2"><AdminField label={copy("Rate name or preset", "ชื่อราคาหรือค่าที่ตั้งไว้")} value={presetSearch} onFocus={() => setPresetOpen(true)} onBlur={() => window.setTimeout(() => setPresetOpen(false), 100)} onChange={(event) => { setPresetSearch(event.target.value); setSelectedPreset(""); setPresetOpen(true); }} placeholder={copy("Search Sunday, Songkran, New Year…", "ค้นหาวันอาทิตย์ สงกรานต์ ปีใหม่…")} autoComplete="off" />
            {presetOpen ? <div className="absolute z-30 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-[#d5d8d4] bg-white p-1 shadow-lg">{filteredPresets.map((preset) => <button key={preset.key} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => choosePreset(preset.key)} className="flex min-h-11 w-full items-center justify-between rounded-lg px-3 text-left text-sm hover:bg-[#edf4f2] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c66f4e]"><span>{preset.labelEn} <span className="text-[#68777a]">/ {preset.labelTh}</span></span><span className="ml-3 text-[10px] text-[#68777a]">{formatRateApplies(preset, locale)}</span></button>)}<button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => choosePreset("custom")} className="flex min-h-11 w-full items-center rounded-lg px-3 text-left text-sm font-semibold text-[#0f6474] hover:bg-[#edf4f2]"><Plus size={13} className="mr-2" /> {copy("Custom dates", "กำหนดวันที่เอง")}</button></div> : null}
          </div>
          {customDates ? <><AdminField label={copy("Custom rate name", "ชื่อราคาแบบกำหนดเอง")} value={localizedInputValue(locale, form.labelEn, form.labelTh)} onChange={(event) => setForm({ ...form, ...localizedInputPatch(locale, "labelEn", "labelTh", event.target.value) })} /><AdminField label={copy("First night", "คืนแรก")} type="date" value={form.startDate} onChange={(event) => setForm({ ...form, startDate: event.target.value })} /><AdminField label={copy("Checkout date", "วันที่เช็กเอาต์")} type="date" value={form.endDate} onChange={(event) => setForm({ ...form, endDate: event.target.value })} helper={copy("The checkout date is not charged.", "ไม่คิดค่าบริการในวันที่เช็กเอาต์")} /></> : null}
          ...
          <div className="self-end rounded-xl border border-[#c9dcd7] bg-white p-3 text-xs leading-5 text-[#405256]"><span className="font-semibold text-[#001e33]">{copy("Applies:", "ใช้กับ:")}</span> {form.recurringDay ? formatRateApplies(form, locale) : form.startDate && form.endDate ? formatRateRange(form.startDate, form.endDate, locale) : selectedPreset ? copy("Choose valid dates", "เลือกวันที่ที่ถูกต้อง") : copy("Choose a preset or Custom dates", "เลือกค่าที่ตั้งไว้หรือกำหนดวันที่เอง")}</div>
        </div>
      ...
      {draft.rates.length ? <AdminSortableList ids={draft.rates.map((rate) => rate.key)} onMove={(active, over) => moveRate(draft.rates.findIndex((item) => item.key === active), draft.rates.findIndex((item) => item.key === over))}><div className="mt-4 space-y-2">{draft.rates.map((rate, index) => { const label = localizedInputValue(locale, rate.labelEn, rate.labelTh) || rate.labelEn || rate.labelTh || copy("Special rate", "ราคาพิเศษ"); return <AdminSortableItem key={rate.key} id={rate.key} className={(dragging) => `flex flex-col gap-2 rounded-lg border px-3 py-2.5 sm:flex-row sm:items-center ${dragging ? "border-[#0f6474] bg-[#f2f8f5] opacity-70" : "border-[#e2e5e1] bg-white"}`}><div className="flex min-w-0 flex-1 items-center gap-2.5"><AdminDragHandle label={copy(`Reorder ${label}`, `จัดลำดับ ${label}`)} /><span className="grid size-7 shrink-0 place-items-center rounded-full bg-[#001e33] text-[11px] font-bold text-white" aria-label={copy(`Rank ${index + 1}`, `ลำดับ ${index + 1}`)}>{index + 1}</span><div className="min-w-0"><p className="truncate text-sm font-semibold text-[#001e33]">{label}</p><p className="mt-0.5 flex flex-wrap items-center gap-x-2 text-[11px] text-[#68777a]"><span className="inline-flex items-center gap-1"><CalendarRange size={12} /> {formatRateApplies(rate, locale)}</span><span className="font-semibold text-[#163038]">฿{rate.nightlyPriceThb.toLocaleString()}/{copy("night", "คืน")}</span></p></div></div><div className="flex gap-1"><button type="button" onClick={() => openRateForm(rate)} className="grid size-11 place-items-center rounded-md text-[#0f6474] hover:bg-[#edf2f0]" aria-label={copy(`Edit ${label}`, `แก้ไข ${label}`)}><Pencil size={14} /></button><button type="button" onClick={() => setDraft((current) => ({ ...current, rates: current.rates.filter((item) => item.key !== rate.key) }))} className="grid size-11 place-items-center rounded-md text-[#9a3f32] hover:bg-[#fff2ed]" aria-label={copy(`Remove ${label}`, `ลบ ${label}`)}><Trash2 size={14} /></button></div></AdminSortableItem>; })}</div></AdminSortableList> : !showRateForm ? <p className="mt-4 text-sm text-[#68777a]">{copy("No special rates. Weekday and weekend prices apply.", "ยังไม่มีราคาพิเศษ ระบบจะใช้ราคาวันธรรมดาและวันหยุดสุดสัปดาห์")}</p> : null}
    </section>
  </div>;
}
...
export function GuestExperienceSection({ draft, amenities, houseRules, setDraft }: { draft: VillaEditorDraft; amenities: Doc<"amenities">[]; houseRules: Doc<"houseRules">[]; setDraft: SetDraft }) {
  ...
  const [custom, setCustom] = useState<CustomAmenityDraft>({ key: "", slug: "", labelEn: "", labelTh: "", icon: "other" });
  useEffect(() => {
      ...
      setDraft((current) => {
        ...
        return {
          ...
          customAmenities: selected
            ...
            : [...current.customAmenities, { key, slug: preset.slug, labelEn: preset.labelEn, labelTh: preset.labelTh, icon: preset.icon }],
        };
      });
      return;
    }
  ...
  function addCustom() { if (!localizedInputValue(locale, custom.labelEn, custom.labelTh).trim() || !custom.icon) return; setDraft((current) => ({ ...current, customAmenities: [...current.customAmenities, { ...custom, key: newKey("amenity") }] })); setCustom({ key: "", slug: "", labelEn: "", labelTh: "", icon: "other" }); setCustomOpen(false); }
...
function RulesEditor({ catalog, rules, onChange }: { catalog: Doc<"houseRules">[]; rules: RuleDraft[]; onChange: (rules: RuleDraft[]) => void }) {
  const { locale, copy } = useAdminLocale();
  const [customOpen, setCustomOpen] = useState(false);
  const [custom, setCustom] = useState<RuleDraft>({ key: "", textEn: "", textTh: "", icon: "other" });
  useEffect(() => {
    if (!customOpen) return;
    function closeOnEscape(event: KeyboardEvent) { if (event.key === "Escape") setCustomOpen(false); }
    document.addEventListener("keydown", closeOnEscape);
  ...
  function toggleCatalog(key: string) {
    ...
    if (key.startsWith("preset:")) {
      ...
      if (preset) onChange([...rules, { key, textEn: preset.textEn, textTh: preset.textTh, icon: preset.icon }]);
      return;
    }
    const item = catalog.find((rule) => rule._id === key);
    if (item) onChange([...rules, { key: newKey("rule"), ruleId: item._id, textEn: item.textEn, textTh: item.textTh, icon: item.icon ?? "other" }]);
  }
  function addCustom() { if (!localizedInputValue(locale, custom.textEn, custom.textTh).trim() || !custom.icon) return; onChange([...rules, { ...custom, key: newKey("rule") }]); setCustom({ key: "", textEn: "", textTh: "", icon: "other" }); setCustomOpen(false); }
...
function formatRateApplies(rate: { startDate?: string; endDate?: string; recurringDay?: "sunday" }, locale: "en" | "th") {
  if (rate.recurringDay === "sunday") return locale === "th" ? "ทุกวันอาทิตย์" : "Every Sunday";
  return formatRateRange(rate.startDate ?? "", rate.endDate ?? "", locale);
}
...
const RATE_PRESETS: RatePreset[] = [
  { key: "sunday", labelEn: "Sunday", labelTh: "วันอาทิตย์", recurringDay: "sunday" },
