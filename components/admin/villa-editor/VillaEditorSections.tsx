...
import { AMENITY_ICON_OPTIONS, AmenityIcon } from "@/lib/amenities";
import { HOUSE_RULE_ICON_OPTIONS, HouseRuleIcon } from "@/lib/house-rules";
...
export function GuestExperienceSection({ draft, amenities, houseRules, setDraft }: { draft: VillaEditorDraft; amenities: Doc<"amenities">[]; houseRules: Doc<"houseRules">[]; setDraft: SetDraft }) {
  ...
  function toggleAmenity(id: Id<"amenities">) { setDraft((current) => ({ ...current, amenityIds: current.amenityIds.includes(id) ? current.amenityIds.filter((item) => item !== id) : [...current.amenityIds, id] })); }
...
function RulesEditor({ catalog, rules, onChange }: { catalog: Doc<"houseRules">[]; rules: RuleDraft[]; onChange: (rules: RuleDraft[]) => void }) {
  ...
  function toggleCatalog(ruleId: string) { const existing = rules.find((rule) => rule.ruleId === ruleId); if (existing) { onChange(rules.filter((rule) => rule.key !== existing.key)); return; } const item = catalog.find((rule) => rule._id === ruleId); if (item) onChange([...rules, { key: newKey("rule"), ruleId: item._id, textEn: item.textEn, textTh: item.textTh, textSource: item.textSource ?? item.textEn, icon: item.icon ?? "other" }]); }
