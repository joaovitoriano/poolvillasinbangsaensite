...
export function PricingSection({ draft, setDraft }: { draft: VillaEditorDraft; setDraft: SetDraft }) {
  ...
  function choosePreset(key: string) {
    ...
    if (key === "custom") { setCustomDates(true); setPresetSearch(copy("Custom dates", "กำหนดวันที่เอง")); setForm((current) => ({ ...current, labelEn: "", labelTh: "", startDate: "", endDate: "" })); return; }
    const preset = RATE_PRESETS.find((item) => item.key === key);
    if (!preset) return;
    setCustomDates(false); setPresetSearch(localizedInputValue(locale, preset.labelEn, preset.labelTh)); setForm((current) => ({ ...current, labelEn: preset.labelEn, labelTh: preset.labelTh, startDate: preset.startDate, endDate: preset.endDate }));
  }
  function saveRate() {
