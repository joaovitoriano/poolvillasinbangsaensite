  ...
  return (
    <DialogContext.Provider value={{ close: onClose }}>
      <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#001326]/55 p-0 backdrop-blur-[2px] sm:items-center sm:p-6" role="presentation" onMouseDown={(event) => { if (closeOnBackdrop && event.target === event.currentTarget) onClose(); }}>
        <div ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="admin-dialog-title" aria-describedby={description ? "admin-dialog-description" : undefined} className={cx("flex max-h-[96dvh] w-full flex-col overflow-hidden rounded-t-3xl bg-[#f5f7f4] shadow-2xl outline-none sm:max-h-[calc(100dvh-3rem)] sm:rounded-3xl", widths[size])}>
